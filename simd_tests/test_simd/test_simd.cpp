...
TEST_CASE( "simd assign", "[simd assign]" ) {
    ...
    SECTION( "assign to array of simd" ) {
        ...
        tpa::assign(tp, std::tuple{isimd_t{1}, std::tuple{1, 2.0, 3ull, 4.1f}});
        auto ps = tpa::to_array(get<0>(tp));
        auto pis = tpa::to_array(get<1>(tp));
        for (int i = 0; i < 4; ++i) REQUIRE(ps[i] == 1.0);
        REQUIRE(pis[0] == 1);
        REQUIRE(pis[1] == 2);
        REQUIRE(pis[2] == 3);
