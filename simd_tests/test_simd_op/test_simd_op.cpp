...
TEST_CASE( "simd binary op", "[simd binary op]" ) {
    ...
    SECTION( "array of simd op" ) {
        ...
        REQUIRE(tpa::to_array(get<0>(r1))[0] == 3);
        REQUIRE(tpa::to_array(get<1>(r1))[1] == 4);
        ...
        REQUIRE(tpa::to_array(get<0>(r2))[0] == 2);
        REQUIRE(tpa::to_array(get<1>(r2))[1] == 4);
    }

    SECTION( "select" ) {
        simd_t simd{1, 2, 3, 4};
        auto r1 = tpa::select(simd < 3, 0.0, isimd_t{5});
        REQUIRE(xsimd::is_batch<decltype(r1)>::value);
        REQUIRE(std::is_same_v<typename decltype(r1)::value_type, double>);
        auto arr1 = tpa::to_array(r1);
        REQUIRE(arr1[0] == 0);
        REQUIRE(arr1[1] == 0);
        REQUIRE(arr1[2] == 5);
        REQUIRE(arr1[3] == 5);
